...
class Storage {
	...
	async updateSuggestion(id: string, update: Partial<Suggestion>): Promise<void> {
		// Convert camelCase to snake_case for the update
		const dbUpdate: any = {}
		for (const [key, value] of Object.entries(update)) {
		  dbUpdate[key.replace(/[A-Z]/g, letter => `_${letter.toLowerCase()}`)] = value
		}
	...
	async saveSuggestion(suggestion: Suggestion): Promise<void> {
	  ...
	  async getWatchHistory(userId: string, limit?: number): Promise<Array<{
		anime: Anime | null
		season: string
		episode: string
		timestamp: number
