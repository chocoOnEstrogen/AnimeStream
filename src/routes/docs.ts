...
router.get('/:category/:slug', async (req, res) => {
    try {
        ...
        const doc = data.docs.find(d => d.category === category && d.slug === slug)
        if (!doc) throw new Error('Document not found')
        
        // Get related documents
        const relatedDocs = doc.metadata.related
            ? data.docs.filter(d => doc.metadata.related?.includes(`${d.category}/${d.slug}`))
            : []
